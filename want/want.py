    ...
    def __upload_token(self, policy):
        try:
            ...
            token = 'UPLOAD_AK_' + self.__type + ' ' + base64_urlsafe_encode(
                self.__ak + ':' + encoded_policy.decode() + ':' + sign).decode()
            return token
    ...
    def __encode_multipart(self, fields):
        ...
        for k, v in fields.items():
            ...
            if not isinstance(v, str):
                v = str(v)
            data.append(('--' + self.boundary).encode())
            data.append(('Content-Disposition: form-data; name="%s"' % k).encode())
            data.append(('').encode())
            data.append((v).encode())
        if content:
            data.append(('--' + self.boundary).encode())
            data.append(('Content-Disposition: form-data; name="content"; filename="%s"' % fields.get('name')).encode())
